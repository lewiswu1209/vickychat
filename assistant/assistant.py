...
class DesktopAssistant(QWidget):
    def __init__(self) -> None:
        ...
        command_menu_items = [
            {"text": "翻译为中文", "connect": self._action_translate_to_chinese},
            {"text": "翻译为英语", "connect": self._action_translate_to_english},
        ]
    ...
    def _action_translate_to_chinese(self):
        self._worker_thread = WorkerThread()
        self._worker_thread.prompt = "{}\n将以上内容翻译成中文：".format(self._dropped_text)
        self._worker_thread.finished.connect(self.on_update_text)
        self._worker_thread.start()

    def _action_translate_to_english(self):
        self._worker_thread = WorkerThread()
        self._worker_thread.prompt = "{}\nTranslate the above into English:".format(self._dropped_text)
        self._worker_thread.finished.connect(self.on_update_text)
        self._worker_thread.start()
